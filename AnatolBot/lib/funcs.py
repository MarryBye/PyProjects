...
class Bot(commands.Bot):
    ...
    async def on_reaction_message(self, member, reaction, guild, message_id, give_type=0):
        ...
        if message_id in config_table:
            for i in range(len(message_table["reactions"])):
                if message_table["reactions"][i] is not reaction:
                    continue
                reaction_role = guild.get_role(message_table["roles"][i])
                if give_type == 0:
                    member.add_roles(reaction_role)
                else:
                    member.remove_roles(reaction_role)
                break
